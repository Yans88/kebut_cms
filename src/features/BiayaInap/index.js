...
import { fetchData, addForm, addData, clearError, confirmDel, closeForm, deleteData } from './biSlice'
...
class BiayaInap extends Component {
    ...
    handleSubmit() {
        ...
        errors.jml = !this.state.selected.jml ? "Jumlah required" : '';
        if (!this.state.selected.id_operator) this.setState({ selected: { ...this.state.selected, id_operator: this.props.user.id_operator } });
    ...
    render() {
        ...
        const frmUser = <Form id="myForm">
            ...
            <Form.Group controlId="jml">
                <Form.Label>Jumlah</Form.Label>
                {errMsg.jml ?
                    (<span className="float-right text-error badge badge-danger">{errMsg.jml}
                    </span>) : ''}
                <NumberFormat
                    name="jml"
                    onChange={this.handleChange.bind(this)}
                    ...
                    <div className="container-fluid">
                        <div className="row mb-2">
                            <div className="col-sm-6">
                                <h1 className="m-0">Biaya Inap</h1>
                            </div>{/* /.col */}
...
const mapStateToProps = (state) => ({
    data: state.biayaInap.data || [],
    totalData: state.biayaInap.totalData,
    isError: state.biayaInap.isError,
    isLoading: state.biayaInap.isLoading,
    isAddLoading: state.biayaInap.isAddLoading,
    showFormAdd: state.biayaInap.showFormAdd,
    errorPriority: state.biayaInap.errorPriority || null,
    contentMsg: state.biayaInap.contentMsg || null,
    showFormSuccess: state.biayaInap.showFormSuccess,
    showFormDelete: state.biayaInap.showFormDelete,
    tipeSWAL: state.biayaInap.tipeSWAL,
...
export default connect(mapStateToProps, mapDispatchToPros)(BiayaInap);