...
export const fetchData = createAsyncThunk(
    ...
    async (param, thunkAPI) => {
        try {
            const response = await axios.post(API_URL + '/kelurahan', param);
            let data = '';
            let _data = response;
            let res = {};
...
export const addData = createAsyncThunk(
    ...
    async (param, thunkAPI) => {
        ...
        try {
            const response = await axios.post(API_URL + '/simpan_kel', param);
            //let data = '';
            let _data = response;
            if (response.status === 200) {
                let dataa = _data.data;
...
export const deleteData = createAsyncThunk(
    ...
    async (param, thunkAPI) => {
        let res = {};
        try {
            const response = await axios.post(API_URL + '/del_kel', param);
            let data = '';
            let _data = response;
            if (response.status === 200) {
                let dataa = _data.data;
...
export const kelSlice = createSlice({
    name: 'kelurahan',
    initialState,
    reducers: {
        addForm: (state) => {
