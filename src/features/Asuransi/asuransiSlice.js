...
export const fetchData = createAsyncThunk(
    ...
    async (param, thunkAPI) => {
        try {
            const response = await axios.post(API_URL + '/asuransi', param);
            let data = '';
            let _data = response;
            let res = {};
...
export const addData = createAsyncThunk(
    ...
    async (param, thunkAPI) => {
        ...
        try {
            const response = await axios.post(API_URL + '/simpan_asuransi', param);
            //let data = '';
            let _data = response;
            if (response.status === 200) {
                let dataa = _data.data;
...
export const deleteData = createAsyncThunk(
    ...
    async (param, thunkAPI) => {
        let res = {};
        try {
            const response = await axios.post(API_URL + '/del_asuransi', param);
            let data = '';
            let _data = response;
            if (response.status === 200) {
                let dataa = _data.data;
...
export const asuransiSlice = createSlice({
    name: 'asuransi',
    initialState,
    reducers: {
        addForm: (state) => {
